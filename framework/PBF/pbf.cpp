...
namespace PBD
{
	...
	void PBF::check_particle_positions() {
		...
		for (int i = 0; i < old_size; i++) {
			...
			if (open_boundary) {
				bool is_out_top = pos[0] <= this->boundary_box_top[0] && pos[1] <= this->boundary_box_top[1] && pos[2] > this->boundary_box_top[2] && pos[2] <= (this->boundary_box_top[2] + 9 * (this->boundary_box_top[2] - this->boundary_box_bottom[2]));
