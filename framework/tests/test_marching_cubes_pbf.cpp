...
/*TEST_CASE("Dam Break PBF with Surface Construction 2", "[Dam Break PBF with Surface Construction 2]")
{
	std::cout << "Testing dam break in PBF with surface construction with particle radius 0.0025 with maxdt 0.0005" << std::endl;
	...
	SECTION("Sparse Marching Cubes") {
		mcparams.sparse = true;
		PBD::PBF pbf(false, false, "../res/dam_break_pbf_mc_sparse_half_r_maxdt0005/dam_break_pbf_mc_sparse_half_r_maxdt0005_", params, mcparams);
		pbf.load_geometry(true, boundary_size, boundary_left, fluid_sizes, fluid_lefts);
		pbf.simulate(4);
		pbf.printStats("Dam break in PBF with sparse marching cubes");
	}
